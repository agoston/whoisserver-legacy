...
char *NH_convert_org(nic_handle_t *nh_ptr)
{
 ...
 int nc=0;
  ...
  nic_components[nc]=strdup("ORG-"); nc++;
  nic_components[nc]=nh_ptr->space; nc++;
...
nic_handle_t *nh_ptr;
     ...
     *nh_ptr_ptr=NULL;
     ...
     if(!(nh_ptr->space=UT_malloc(ptr-nic+1))) die;
     strncpy(nh_ptr->space, nic+4, ptr-nic-4); *(nh_ptr->space+(ptr-nic-4))='\0';
...
/*printf("DEBUG: get_range_org: entered with nic_id=[%li]\n", nic_id);*/
  ...
  g_string_sprintf(query, "SELECT range_id, range_end " 
                          ...
                          "AND source='%s' "
                          "AND range_end>=%ld ",
                          prange->space, prange->source, nic_id);
...
int sql_err;
  ...
  /* Do range check */
  if ( p_newrange->end > MAX_RANGE ) return(-1);
  ...
  if(commit_now) {
    g_string_sprintf(query, "UPDATE organisation_id "
     ...
     g_string_sprintf(query, "INSERT organisation_id "
                             "SET thread_id=%d, range_id=%ld, space='%s', source='%s', range_end=%ld ",
                             NH_INSERT, range_id, p_newrange->space, p_newrange->source, p_newrange->end);  
  } 
...
long range_id;
  ...
  if(commit_now){
    g_string_sprintf(query, "INSERT organisation_id "
                            "SET thread_id=0,  range_id=%ld, space='%s', source='%s', range_end=%ld ",
                            range_id, p_range->space, p_range->source, p_range->end);
  }else{
    g_string_sprintf(query, "INSERT organisation_id "
                            "SET thread_id=%d, range_id=%ld, space='%s', source='%s', range_end=%ld ",
                             NH_INSERT, range_id, p_range->space, p_range->source, p_range->end);
  }
