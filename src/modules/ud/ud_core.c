...
char *host;
  ...
  if (*delim)*delim='\0'; else return(NULL);
  delim++; 

/* convert the type      */
  if(g_strcasecmp(refer, S_RIPE)==0)*type=RF_RIPE;
   else if(g_strcasecmp(refer, S_INTERNIC)==0)*type=RF_INTERNIC;
    else if(g_strcasecmp(refer, S_SIMPLE)==0)*type=RF_SIMPLE;
     else if(g_strcasecmp(refer, S_CLIENTADDERSS)==0)*type=RF_CLIENTADDRESS;
...
char *result;
 ...
 if (condition)g_string_append(query, condition);
...
/* Check if the set protects itself with mbrs-by-ref attribute */
   g_string_sprintf(query,"SELECT COUNT(*) FROM mbrs_by_ref, %s "
			  ...
			  "AND %s.%s='%s' ",
			  set_name, set_name, set_name, set_name, attribute_value);
   ...
   if (strcmp(qresult, "0")==0){
	   /* there is no mbrs-by-ref attribute - so we cannot go ahead */
           LG_log(tr->src_ctx, LG_DEBUG, "[%ld] membership by reference is not allowed (no mbrs-by-ref) [%d:%s]", tr->transaction_id, attribute_type, attribute_value);
	   UT_free(qresult);  
	   g_string_free(query, TRUE);
           return(1);
   }
   else UT_free(qresult);
...
*/   
    g_string_sprintf(query,"SELECT %s.object_id FROM mbrs_by_ref, %s "
			   ...
			   set_name, set_name, set_name, set_name, set_name, attribute_value);
...
/* we search only mnt_by.thread_id!=0 to check against new/updated mnt-by attribute */
    g_string_sprintf(query, "SELECT mbrs_by_ref.object_id FROM %s, mbrs_by_ref, mnt_by "
 			    "WHERE mbrs_by_ref.mnt_id=mnt_by.mnt_id "
    			    ...
    			    set_name, tr->object_id, set_name, set_name, set_name, 
    ...
    if (qresult) {
	 ...
	 return(0);
         ...
         LG_log(tr->src_ctx, LG_DEBUG, "[%ld] membership by reference is not authorised [%d:%s]", tr->transaction_id, attribute_type, attribute_value);
	 g_string_free(query, TRUE);
	 return(1);
    }
 }/* auth_member_of()  */
...
/* Allocate resourses. We cannot use tr->query because it is in use */
 ...
 /* Record dummy's object_id, it'll be needed in commit/rollback */
  ...
  sprintf(str_id, "%ld", tr->object_id);
  timestamp=time(NULL);
  g_string_sprintf(query, "INSERT INTO last SET thread_id=%d, object_id=%ld, timestamp=%ld, object_type=%d, object='DUMMY for %s'", 
      ...
      /* go ahead and register */
        if(NH_check(nh_ptr, tr->sql_connection)>0){
          ...
          if(nhres == -1) {
           ...
           LG_log(tr->src_ctx, LG_INFO, "[%ld] nic handle [%s]: wrong or already in use", tr->transaction_id, attribute_value);
...
/* stored in the main table, like inetnum, filter-set, etc. */
   if((IS_STANDALONE(tr->mode))&&(DF_get_update_query_type(attribute_type)!=UD_MA_U2)) return;
...
/* Do some additional processing for reverse domains */
   ...
   switch (DF_get_update_query_type(attribute_type)) {
         case UD_MAIN_: g_string_sprintf(tr->query, query_fmt, tr->thread_upd, tr->object_id);
                        break;
	 case UD_MA_PR: 
	 ...
	 case UD_MA_U2: /* save the new value of the attribute for commit*/
		  ...
		  /* local-as(inet-rtr) attributes, as they are another field in the record */
		      ...
		      /* for 2-pass loader we need to update the field as we have no commit */
		          g_string_sprintf(tr->query, query_fmt, DF_get_class_sql_table(tr->class_type), 0, attribute_value, tr->object_id);
		        }
		        else {
	 ...
	 case UD_AX_PR:
	 		...
	 		g_string_sprintf(tr->query, query_fmt, tr->thread_upd, tr->object_id, 
	 			get_ref_id(tr, "person_role", "nic_hdl", attribute_value, condition));
	 		break;
	 case UD_AX_MT: 
	 		if (!IS_DUMMY_ALLOWED(tr->mode))condition="AND dummy=0 "; else condition=NULL;
	 		g_string_sprintf(tr->query, query_fmt, tr->thread_upd, tr->object_id, 
   ...
   case UD_AX_OA: 
      if (!IS_DUMMY_ALLOWED(tr->mode))condition="AND dummy=0 "; else condition=NULL;
      g_string_sprintf(tr->query, query_fmt, tr->thread_upd, tr->object_id, 
         get_ref_id(tr, "organisation", "organisation", attribute_value, condition));
      break;
 	 case UD_AX_IT: 
	 		if (!IS_DUMMY_ALLOWED(tr->mode))condition="AND dummy=0 "; else condition=NULL;
	 		g_string_sprintf(tr->query, query_fmt, tr->thread_upd, tr->object_id, 
	 			get_ref_id(tr, "irt", "irt", attribute_value, condition));
	 		break;
	 ...
	 case UD_AX_MO: 
			...
			if (!IS_DUMMY_ALLOWED(tr->mode))condition="AND dummy=0 "; else condition=NULL;
			g_string_sprintf(tr->query, query_fmt, tr->thread_upd, tr->object_id, 
					get_ref_id(tr, set_name, set_name, attribute_value, condition));
			break;	    			
    	 case UD_AX_MR:
   ...
   /* Execute the query */
	...
	die;
     ...
     /* Query OK */
	  ...
	  tr->succeeded=0;
          LG_log(tr->src_ctx, LG_INFO, "[%ld] membership by reference is not allowed [%d:%s]", tr->transaction_id, attribute_type, attribute_value);
...
/*XXX */
  ...
  /* for pn, mt, rs, as update dummy field */
   ...
   }else if((tr->class_type==C_AS)  || (tr->class_type==C_RS) ||
