...
int get_NRTM_fd(struct _nrtm *nrtm, int upto_last, char *source)
{
	...
	if ((ret = SK_connect(&sockfd, nrtm->server, nrtm->port, 30)) == SK_OK) {	/* this is a network stream */
		...
		LG_log(ud_context, LG_ERROR, "cannot connect to %s:%d; neither tcp connection nor file", nrtm->server, nrtm->port);
		return -1;
	}
	...
	do {
		...
		if (do_update) {
			...
			LG_log(src_ctx, LG_INFO,
				...
				UD_TAG, nrtm->server, nrtm->port, nrtm->current_serial);
