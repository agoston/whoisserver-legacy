...
static int process_transaction(UD_stream_t *ud_stream, GString *obj_buff, int operation, long transaction_id,
...
static long ud_replace_autokey(Transaction_t *tr, rpsl_object_t *object, int *IS_X509) {
	...
	if (keycert_id >= 0) {
		...
		else { /* this is "X509-NNN" name */
			key = key_cert_submitted + 5; // "X509-"
			keycert_id_submitted = atol(key);
			if (keycert_id_submitted == 0) { /* reject X509-0 */
				keycert_id = 0;
			} else {
...
static void reorder_attr(void *element_data, void *ptr) {
	...
	if (attr)
		rpsl_object_add_attr(object, attr, 1, NULL);
	else
		die;
}
...
static int report_transaction(Transaction_t *tr, long transaction_id, Log_t *log, ut_timer_t *psotime, char *reason) {
	...
	if (tr->succeeded==0) {
		result=tr->error;
		log->num_failed++;
		LG_log(tr->src_ctx, LG_INFO, "[%ld] %.2fs FAILED [%s:%s][%s]", transaction_id, timediff, class_name,
		        primary_key, reason);
		if (result & ERROR_U_OBJ)
			LG_log(tr->src_ctx, LG_DEBUG, "[%ld] object: referential integrity error", transaction_id);
		if (result & ERROR_U_AUT)
			LG_log(tr->src_ctx, LG_DEBUG, "[%ld] object: authentication error", transaction_id);
		if (result & ERROR_U_BADOP)
			LG_log(tr->src_ctx, LG_DEBUG, "[%ld] object: unsupported operation", transaction_id);
		if (result & ERROR_U_COP)
			LG_log(tr->src_ctx, LG_DEBUG, "[%ld] object: conflicting operation", transaction_id);
		if (result & ERROR_U_NSUP)
			LG_log(tr->src_ctx, LG_DEBUG, "[%ld] object: this type is not supported", transaction_id);
		LG_log(tr->src_ctx, LG_DEBUG, "[%ld] object: details [%s]", transaction_id, (tr->error_script)->str);
		result=1; /* # of failures */
	} else {
		result=0;
		log->num_ok++;
		LG_log(tr->src_ctx, LG_INFO, "[%ld] %.2fs OK     [%s:%s][%s]", transaction_id, timediff, class_name,
		        primary_key, reason);
		LG_log(tr->src_ctx, LG_DEBUG, "[%ld] object: details [%s]", transaction_id, (tr->error_script)->str);
	}
...
int UD_process_stream(UD_stream_t *ud_stream, LG_context_t *src_ctx) {
	...
	if (IS_STANDALONE(ud_stream->ud_mode) || (!IS_UPDATE(ud_stream->ud_mode))) {
		...
		LG_log(src_ctx, LG_INFO, "%s average processing time %7.4f obj/s (%6.2f obj/min)", UD_TAG, obj_second10,
		        obj_second10*60);
