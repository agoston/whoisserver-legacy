...
inline int ip_rang_validate(ip_range_t *rangptr) {
  ...
  switch (rangptr->begin.space) {
  	case IP_V4:
    	if (rangptr->begin.words[0] > rangptr->end.words[0]) return IP_INVRAN;
    	break;

    case IP_V6: {
			int i;
			for (i=0; i < 4; i++) {
				if (rangptr->begin.words[i] > rangptr->end.words[i]) return IP_INVRAN;
				if (rangptr->begin.words[i] < rangptr->end.words[i]) break;
			}
    	break;
    }
  }
...
gboolean IP_addr_isnull(ip_addr_t *addrptr) {
	switch (addrptr->space) {
		case IP_V4:
			if (addrptr->words[0] == 0) return TRUE;
			else return FALSE;

		case IP_V6: {
			int i;
			for (i = 0; i < 4; i++) {
				if (addrptr->words[i] != 0) return FALSE;
			}
			return TRUE;
		}
	}
...
int
IP_pref_a2v6_32(const char *avalue, ip_prefix_t *pref,
