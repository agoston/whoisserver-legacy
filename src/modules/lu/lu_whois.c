...
const char * const LU_ret_to_string[] = { "LU_ERROR", "LU_INVARG", "LU_OKAY" };
 ...
 */
void lu_whois_cleanup(LU_server_t *server) {
 ...
 */
static gboolean lu_whois_query(LU_whois_info_t *whois_info, const gchar *query, GList **objects) {
    int connect_timeout;
    int sk_err;
    gchar *tmp;
    gchar *fixed_query;
    ...
    *objects = NULL;
    ...
    if (whois_info->socket < 0) {
        ...
        sk_err = SK_connect(&whois_info->socket, whois_info->hostname, whois_info->port, connect_timeout);
        if (sk_err != SK_OK)
        {
        ...
        if (split_reply[i][0] == '%') {
            ...
            if (strncmp(split_reply[i], "%ERROR:", 7) == 0) {
                ...
                if (tmp_error_code != 101) {
                    LG_log(lu_context, LG_WARN, "lu_whois_query: server returned code %d", tmp_error_code);
...
/*
 ...
 obj      - object
 */
static void lu_log_object(rpsl_object_t *obj) {
    const GList *attr;
    const GList *p;

    attr = rpsl_object_get_all_attr(obj);
    for (p = attr; p != NULL; p = g_list_next(p)) {
        LG_log(lu_context, LG_DEBUG, "%s:%s", rpsl_attr_get_name(p->data), rpsl_attr_get_value(p->data));
    }
}
...
GString* glist_to_string(GList *list, GString *string) {
    ...
    p = list;
    if (p != NULL) { /* list is defined, prepare the string */
...
LU_ret_t lu_whois_inverse_lookup(LU_server_t *server, GList **objects, GList *types, const gchar *key, GList *inverse_keys, const gchar *source) {
    GList *p;
    ...
    if (types_string != NULL)
    {
        LG_log(lu_context, LG_DEBUG, "lu_whois_inverse_lookup: types=\"%s\", key=\"%s\", \
                                 inverse_keys=\"%s\", source=\"%s\"", types_string->str, key, inverse_keys_string->str, source);
        ...
        LG_log(lu_context, LG_DEBUG, "lu_whois_inverse_lookup: types=all, key=\"%s\", \
                                 inverse_keys=\"%s\", source=\"%s\"", key, inverse_keys_string->str, source);
    ...
    *objects = NULL;
    ...
    /* return result */
    LG_log(lu_context, LG_FUNC, "<lu_whois_inverse_lookup: exiting with value [%s]", LU_ret2str(ret_val));
    return ret_val;
...
/*
 ...
 Logging perfomed via LG module.
 */
LU_ret_t lu_whois_get_object(LU_server_t *server, rpsl_object_t **dbase_obj, const rpsl_object_t *obj, const gchar *source) {
    const gchar *class;
    gchar *key;
    GList *source_attr;
    gchar *lookup_source;
    ...
    if (source == NULL)
    {
        ...
        if (g_list_length(source_attr) != 1) {
            ...
            LG_log(lu_context, LG_FUNC, "<lu_whois_get_object: exiting with value [%s]", LU_ret2str(LU_ERROR));
            return LU_ERROR;
        }
    ...
    /* return */
    LG_log(lu_context, LG_FUNC, "<lu_whois_get_object: exiting with value [%s]", LU_ret2str(ret_val));
    return ret_val;
}
 ...
 */
static gboolean has_hierarchical_name(const gchar *class) {
...
LU_ret_t lu_whois_check_overlap(LU_server_t *server, GList **overlap, const rpsl_object_t *obj, const gchar *source) {
    LU_ret_t ret_val;
    ...
    *overlap = NULL;
    ...
    if (source == NULL)
    {
        ...
        if (g_list_length(source_attr) != 1) {
            /* lg(error, missing source) */
            rpsl_attr_delete_list(source_attr);
            LG_log(lu_context, LG_FUNC, "<lu_whois_check_overlap: exiting with value [%s]", LU_ret2str(LU_ERROR));
            return LU_ERROR;
        }
        lookup_source = rpsl_attr_get_clean_value(source_attr->data);
    } else {
    ...
    LG_log(lu_context, LG_FUNC, "<lu_whois_check_overlap: exiting with value [%s]", LU_ret2str(ret_val));
    g_free(range);
    g_strfreev(split);
    UT_free(lookup_source);
    return ret_val;
...
/*
 ...
 Logging perfomed via LG module.
 */
LU_ret_t lu_whois_get_parents(LU_server_t *server, GList **parents, const rpsl_object_t *obj, const gchar *source) {
    GList *source_attr;
    gchar *lookup_source;
    const gchar *class;
    gchar *key;
    ...
    *parents = NULL;
    ...
    if (source == NULL)
    {
        ...
        if (g_list_length(source_attr) != 1) {
            /* lg(error, missing source) */
            rpsl_attr_delete_list(source_attr);
            LG_log(lu_context, LG_FUNC, "<lu_whois_get_parents: exiting with value [%s]", LU_ret2str(LU_ERROR));
            return LU_ERROR;
        }
        lookup_source = rpsl_attr_get_clean_value(source_attr->data);
    } else {
    ...
    if (has_hierarchical_name(class)) {
        ...
        if (p == NULL)
        {
            ...
            *p = '\0';
            ...
            /* preform lookup */
            query = g_strdup_printf("-G -B -s %s -r -T %s %s", lookup_source, parent_type, parent_name);
        }UT_free(parent_name);
        ...
        while ((ret_val == LU_OKAY) && (parent_name != NULL) && (*parents == NULL)) {
            ...
            query = g_strdup_printf("-G -B -s %s -R -r -T domain %s", lookup_source, parent_name);
            query_ret = lu_whois_query(server->info, query, parents);
            g_free(query);
            if (!query_ret) {
                ret_val = LU_ERROR;
    ...
    rpsl_attr_delete_list(source_attr);
    UT_free(lookup_source);
    LG_log(lu_context, LG_FUNC, "<lu_whois_get_parents: exiting with value [%s]", LU_ret2str(ret_val));
    return ret_val;
}
 ...
 */
LU_ret_t lu_whois_get(LU_server_t *server, gchar *query_str, gchar *source, GList **result, const gchar *key) {
    gchar *query;
    gint ret_val;
    gboolean query_ret;
